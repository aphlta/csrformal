...
- Smstateen 未列条款（全部 stateen 位置 1，等于这些门不参与）
- Smcdeleg 间接窗口（排除 sireg*/vsireg*/mireg* 地址段）
- custom CSR 的 stateen.C 门控（C=1，不把 RTL 自定义地址译进规格）
...
# lint / spec-drift 收录这些条款。vti_stc 无 rule_id，靠 note 过 lint。
...
# Smcdeleg / Smcsrind 间接窗口：本轮不建模，假设里排除这些地址。
# 范围按手册分配（S/VS/M 间接 CSR 落在 0x15x / 0x25x / 0x35x）。
IND_S = (0x150, 0x157)
IND_VS = (0x250, 0x257)
IND_M = (0x350, 0x357)

# 与 case 层 clean() 对齐的 stateen 端口短名（前缀 io_in_xstateen_）。
# 置 1 是为了关掉未建模的 Smstateen 路径，不是把 stateen 语义写进规格。
STATEEN_PORTS = [
...
def eq_assumes() -> List[str]:
    ...
    a += [f"(= io_in_xstateen_{s} true)" for s in STATEEN_PORTS]
    return a
